...
Eigen::Affine3f
mapOptimization::pclPointToAffine3fCameraToLidar(PointTypePose thisPoint)
{ // camera frame to lidar frame
...
bool mapOptimization::LMOptimization(int iterCount)
{
  ...
  float deltaR = sqrt(
      pow(pcl::rad2deg(matX.at<float>(0, 0)), 2) + pow(pcl::rad2deg(matX.at<float>(1, 0)), 2) + pow(pcl::rad2deg(matX.at<float>(2, 0)), 2));
  float deltaT = sqrt(
