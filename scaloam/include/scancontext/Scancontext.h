...
MatrixXd circshift(MatrixXd &_mat, int _num_shift); // 把矩阵循环右移shift个单位
std::vector<float> eig2stdvec(MatrixXd _eigmat); // convert eigen matrix to vector
...
class SCManager
{
 public:
  ...
  Eigen::MatrixXd makeRingkeyFromScancontext(Eigen::MatrixXd &_desc); // 由scan context计算ring key，Nx1
  Eigen::MatrixXd makeSectorkeyFromScancontext(Eigen::MatrixXd &_desc);

  // _vkey1、_vkey2是两个sector key; 对_vkey2做循环偏移,计算与_vkey1最佳匹配时的偏移量
  int fastAlignUsingVkey(MatrixXd &_vkey1, MatrixXd &_vkey2);
  //??? "d" (eq 5) in the original paper (IROS 18)
  double distDirectSC(MatrixXd &_sc1, MatrixXd &_sc2);
  // 计算两个scan context之间的距离; "D" (eq 6) in the original paper (IROS 18)
 ...
 public:
  // hyper parameters ()
  // lidar height : add this for simply directly using lidar scan in the lidar local coord (not robot base coord)
  ...
  // tree
  // simply just keyframe gap (related with loopClosureFrequency in yaml),
  // but node position distance-based exclusion is ok.
  ...
  // empirically 0.1-0.2 is fine (rare false-alarms) for 20x60 polar context
  ...
  // config
  // i.e., remaking tree frequency, to avoid non-mandatory every remaking, to save time cost
  // in the LeGO-LOAM integration, it is synchronized with the loop detection callback (which is 1Hz), so it means the
