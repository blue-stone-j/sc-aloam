                                          ...
                                          std::vector<int> &matched_frame_id, Eigen::Isometry3d &odom_in)
{
  ...
  odom_original_arr.push_back(last_pose3.between(pose3_current)); //@jxl: delta_transform: last opted --->curr unOpted
  ...
  { // 检测到闭环，个数为1
    ...
    for (int i = 0; i < (int)matched_frame_id.size( ); i++)
    {
      ...
      //@jxl: transform: history frame to curr frame delta_transform

      // 用curr帧和发生闭环的history帧(corner to corner, surf to surf)对initial guess 提纯, 计算闭环约束。
      // true：确实发生了闭环;  false: 给的闭环是假闭环
      // 作者没有实现论文中的temporal consistency check
...
/**
 * @param current_id: 当前帧id
 * @param matched_id: 发生闭环的history id
 * @param transform:  history frame to curr frame delta_transform
 */
bool ISCOptimizationClass::geometryConsistencyVerification(int current_id, int matched_id, Eigen::Isometry3d &transform)
{
  ...
  //  transform_pose.translation() = Eigen::Vector3d(0,0,10); //为啥要覆盖平移，这么做的原因是什么？
  ...
  //  *loop_candidate_pc+=*cloud_temp; //loop_candidate_pc:没有用
  //  loop_map_pc = map_surf;
  //  *loop_map_pc += *map_edge; //loop_map_pc:没有用
...
/**
 * @param pc_source_edge: history id 附近的帧转到history frame下
 * @param pc_source_surf: history id 附近的帧转到history frame下
 * @param pc_target_edge: curr edge points
 * @param pc_target_surf: curr surf points
 * @param transform: history frame to curr frame delta_transform initial guess,
